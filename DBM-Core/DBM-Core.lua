...
do
	...
	local function onWhisper(msg, sender, isRealIdMessage)
		...
		if msg == "status" and #inCombat > 0 and DBM.Options.StatusEnabled and (isRealIdMessage or checkForFriend(sender)) then
		...
		elseif #inCombat > 0 and DBM.Options.AutoRespond and (isRealIdMessage and (not isOnSameServer(sender) or not DBM:GetRaidUnitId(select(5, BNGetFriendInfoByID(sender)))) or not isRealIdMessage and not DBM:GetRaidUnitId(sender) and checkForFriend(sender)) then
			if not difficultyText then -- prevent error when timer recovery function worked and etc (StartCombat not called)
				savedDifficulty, difficultyText, difficultyIndex, LastGroupSize = DBM:GetCurrentInstanceDifficulty()
			end
			local mod
